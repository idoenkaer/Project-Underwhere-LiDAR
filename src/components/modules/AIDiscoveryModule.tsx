...
// Helper to parse metadata from the AI response
const parseMetadata = (responseText: string) => {
    const metadata: Record<string, string> = {};
    const metadataBlock = responseText.split('---')[0];
    const lines = metadataBlock.split('\n');
    lines.forEach(line => {
        const parts = line.split(':');
        if (parts.length >= 2) {
            const key = parts[0].replace(/-\s\*\*/, '').replace(/\*\*/, '').trim();
            const value = parts.slice(1).join(':').trim();
            metadata[key] = value;
        }
    });
    return {
        model: metadata['Primary Model'] || 'N/A',
        confidence: parseFloat(metadata['Overall Confidence']?.replace('%', '')) || 0,
        uncertainty: parseFloat(metadata['Uncertainty Score']?.replace('%', '')) || 0,
    };
};


const AIDiscoveryModule: React.FC = () => {
  const { addLog, addAlert, walkthroughStep, completeWalkthroughStep, setActiveModule } = useUIStateContext();
  ...
  const [reportMetadata, setReportMetadata] = useState({ model: 'N/A', confidence: 0, uncertainty: 0 });
  ...
  const handleQuerySubmit = async (query: string) => {
    ...
    if (walkthroughStep === 'ai-discovery-query') {
    ...
    setReportMetadata({ model: 'N/A', confidence: 0, uncertainty: 0 });
  ...
  useEffect(() => {
      if (!isLoading && aiResponse) {
          try {
            setReportMetadata(parseMetadata(aiResponse));
          } catch(e) {
            console.error("Failed to parse AI metadata:", e);
            addAlert("Could not parse AI response metadata. The format may be invalid.", "error");
          }
      }
  }, [isLoading, aiResponse, addAlert]);
  ...
  const handleViewBenchmark = () => {
    const modelVersion = reportMetadata.model.split('-').pop() || 'unknown';
    addLog(`Navigating to Validation module for model: ${modelVersion}`);
    setActiveModule(Module.VALIDATION, { modelVersion });
  };

  return (
    <>
      {walkthroughStep === 'tour-complete' && (
        <WalkthroughTip
          targetRef={{ current: document.body }} // Use body for a centered final tip
          onDismiss={completeWalkthroughStep}
          title="Tour Complete!"
          position="bottom"
        >
          You've completed the core workflow from scan to AI insight. Feel free to explore other modules like `Export & Share` to see how you can use these results.
        </WalkthroughTip>
      )}
      {showExplanation && (
          <ExplanationModal
      ...
      <div className="flex flex-col h-[calc(100vh-10rem)] max-w-4xl mx-auto animate-fadeIn">
        <div className="flex-1 bg-bg-secondary/50 rounded-t-sm border border-b-0 border-green-dark p-6 flex flex-col" ref={responseContainerRef}>
          <div className="flex-grow overflow-y-auto">
            {aiResponse ? (
                <>
                   {reportMetadata.confidence > 0 && (
                        <div className="bg-bg-primary/50 p-4 rounded-sm border border-green-dark mb-6 animate-fadeInUp">
                            <h3 className="text-md font-semibold text-text-accent mb-4 flex items-center font-mono">
                                <ShieldCheckIcon className="h-5 w-5 mr-2 text-green-bright" />
                                Trust & Transparency
                            </h3>
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm items-center">
                               <div className="flex flex-col items-center justify-center">
                                    <div className="relative h-24 w-24">
                                        <GaugeIcon className="h-full w-full text-green-dark" />
                                        <div 
                                            className="absolute inset-0 transition-transform duration-1000" 
                                            style={{transform: `rotate(${(reportMetadata.confidence / 100) * 180 - 90}deg)`}}
                                        >
                                            <div className="h-1/2 w-1 bg-green-bright absolute top-0 left-1/2 -translate-x-1/2 origin-bottom scale-y-90"></div>
                                        </div>
                                        <div className="absolute bottom-4 left-1/2 -translate-x-1/2 font-mono text-2xl font-bold text-green-bright">
                                            {reportMetadata.confidence}
                                            <span className="text-xs">%</span>
                                        </div>
                                    </div>
                                    <p className="font-bold text-green-muted text-xs mt-1 font-mono">CONFIDENCE SCORE</p>
                                </div>
                                <div className="space-y-3">
                                    <div className="space-y-1">
                                        <p className="font-bold text-green-muted text-xs font-mono">MODEL VERSION</p>
                                        <p className="font-mono text-green-bright">{reportMetadata.model}</p>
                                    </div>
                                    <div className="space-y-1">
                                        <p className="font-bold text-green-muted text-xs font-mono flex items-center"><ShieldExclamationIcon className="h-4 w-4 mr-1"/>UNCERTAINTY SCORE</p>
                                        <p className="font-mono text-warning">{reportMetadata.uncertainty}%</p>
                                    </div>
                                </div>
                                <div>
                                    <button onClick={handleViewBenchmark} className="w-full p-3 bg-data-blue/20 text-data-blue font-mono rounded-sm border-2 border-data-blue/50 hover:bg-data-blue/40 transition text-sm flex items-center justify-center space-x-2">
                                        <ChartBarIcon className="h-4 w-4" />
                                        <span>View Model Benchmark</span>
                                    </button>
                                </div>
                            </div>
                        </div>
                   )}
                    <SimpleMarkdown text={aiResponse.split('---').slice(1).join('---')} />
                </>
            ) : (
                <div className="flex flex-col items-center justify-center h-full text-center text-green-muted">
                    ...
                    {isConfigured ? (
                        <div className="relative w-full">
                           {walkthroughStep === 'ai-discovery-query' && suggestedQueriesRef.current && (
                               <WalkthroughTip
                                   targetRef={suggestedQueriesRef}
                                   onDismiss={completeWalkthroughStep}
                                   title="Step 3: Ask the AI"
                                   position="top"
                               >
                                   Finally, use the AI to interpret your results. Click one of the suggested queries to generate a full semantic report and complete the tour.
                               </WalkthroughTip>
                           )}
                           <p className="mt-2 max-w-md mx-auto">Analyze processed scan data to generate a publication-ready scientific report.</p>
                           <div ref={suggestedQueriesRef} className="mt-6 border-t border-green-dark w-full max-w-sm mx-auto pt-4">

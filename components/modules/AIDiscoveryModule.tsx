...
const SimpleMarkdown: React.FC<{ text: string }> = ({ text }) => {
    ...
    const html = safeText
        .replace(/^### (.*$)/gim, '<h3 class="text-lg font-semibold text-cyan-300 mb-2 mt-4">$1</h3>')
        .replace(/^## (.*$)/gim, '<h2 class="text-xl font-bold text-cyan-200 mb-3 mt-4">$1</h2>')
        .replace(/^# (.*$)/gim, '<h1 class="text-2xl font-extrabold text-cyan-100 mb-4 mt-4">$1</h1>')
...
const AIDiscoveryModule: React.FC = () => {
  ...
  const parsedResponse = useMemo(() => {
    ...
    // Regexes to find sections, accepting markdown headers or bolded text.
    const directAnalysisMatch = aiResponse.match(/(?:#+\s*Direct Analysis[:\s]*|^\s*\*\*Direct Analysis:\*\*\s*)([\s\S]*?)(?=#+\s*Suggested Hypotheses|^\s*\*\*Suggested Hypotheses:\*\*|$)/im);
    const hypothesesMatch = aiResponse.match(/(?:#+\s*Suggested Hypotheses[:\s]*|^\s*\*\*Suggested Hypotheses:\*\*\s*)([\s\S]*?)(?=#+\s*Relevant Literature|^\s*\*\*Relevant Literature \(Hypothetical\):\*\*|$)/im);
    const literatureMatch = aiResponse.match(/(?:#+\s*Relevant Literature(?: \(Hypothetical\))?[:\s]*|^\s*\*\*Relevant Literature \(Hypothetical\):\*\*\s*)([\s\S]*)/im);

    if (directAnalysisMatch) sections['analysis'] = directAnalysisMatch[1].trim();
    if (hypothesesMatch) sections['hypotheses'] = hypothesesMatch[1].trim();
    if (literatureMatch) sections['literature'] = literatureMatch[1].trim();
